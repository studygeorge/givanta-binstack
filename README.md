...
- **Лоток** ходит дном в пазу между двумя полозьями (паз 3,2 мм, дно 2,4 —
  зазор 0,4 на сторону). Наклон 12° работает на удержание: сам лоток внутрь
  не сползает, а ход назад ограничен задними штангами.
- **Детент-упор** — главное, что держит лоток от произвольного выезда. На
  переднем конце каждого нижнего полоза напечатан бугорок 0,7 мм с пологим
  въездом и крутым сходом. Дно лотка перещёлкивается через него с ощутимым
...
![Работа детента](docs/drawings/detent.svg)
...
| Файл | Что это | Размер |
|---|---|---|
| [print/tk-rail-coupon.stl](print/tk-rail-coupon.stl) | купон: три пары полозьев с зазорами **2,9 / 3,2 / 3,5 мм** (маркировка 1/2/3 точками), у каждого свой детент | 70×80×10, 35 см³ |
| [print/tk-wedge-12deg.stl](print/tk-wedge-12deg.stl) | клин 12° — рабочий наклон полозьев | 30×70×15, 16 см³ |
| [print/tk-slider.stl](print/tk-slider.stl) | слайдер: пластина 2,4 мм (толщина дна лотка) с ручкой | 20×64×8, 4 см³ |

Протокол:
1. На плоском столе прогони слайдер по каждому пазу: ход свободный, без
   закусывания, и на детенте — внятный щелчок, а не заклинивание.
2. Поставь купон на клин полозьями вдоль склона. Слайдер за детентом должен
   **стоять намертво**, даже если стукнуть по столу, и трогаться только от руки.
3. Выбери паз без закусывания и без болтанки: это твой рабочий зазор. Если
   победил не 3,2 — скажи, и полозья пересчитываются одним параметром.
